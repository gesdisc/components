...
function getUTCDate(date, endOfDay = false) {
  const utcDate = import_dayjs.default.utc(date).toDate();
  if (endOfDay)
    utcDate.setUTCHours(23, 59, 59, 999);
  return utcDate;
}
function formatDate(date, format) {
  if (!import_dayjs.default.isDayjs(date)) {
    date = import_dayjs.default.utc(date);
  }
  return date.format(format);
}
function isDateRangeContained(start1, end1, start2, end2) {
